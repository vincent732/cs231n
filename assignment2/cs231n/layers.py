...
def affine_backward(dout, cache):
    ...
    dx = dout.dot(w.T).reshape(x.shape)#(N, D) = (N, M) * (M, D)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for h in range(outH):
                for w in range(outW):
                    out[n, c, h, w] = np.max(x[n, c, h*s:h*s+hh, w*s:w*s+ww])
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for c in range(C):
            for h in range(outH):
                for w in range(outW):
                    ...
                    window = window.reshape(hh * ww)
                    window_out = np.zeros_like(window)
                    window_out[np.argmax(window)] = 1
                    window_out = window_out.reshape(hh, ww)
                    dx[n, c, h*s:h*s+hh, w*s:w*s+ww] = window_out * dout[n, c, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout_head = dout.transpose((0, 2, 3, 1))
